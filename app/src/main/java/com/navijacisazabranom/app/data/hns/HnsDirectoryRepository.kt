...
class HnsDirectoryRepository @Inject constructor() : DirectoryRepository {
    ...
    override suspend fun getOrganizacije(): Result<List<Organizacija>> = withContext(Dispatchers.IO) {
        runCatching {
            val json = fetchJson("${HnsConfig.BASE_URL}/handlers/getOrganizations/") { connection ->
                connection.data("season", HnsConfig.tekucaSezona())
            }
            HnsDirectoryParser.parseOrganizacije(json)
        }
    }

    override suspend fun getNatjecanja(organizacijaId: String): Result<List<Natjecanje>> = withContext(Dispatchers.IO) {
        runCatching {
            val json = fetchJson("${HnsConfig.BASE_URL}/handlers/getCompetitions/") { connection ->
                connection
                    .data("season", HnsConfig.tekucaSezona())
